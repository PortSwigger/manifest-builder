...
@mock.patch("manifest_builder.cli.get_helm_version", return_value="v3.0.0")
def test_main_reports_manifest_errors_without_output_dir_paths(
    mock_get_helm_version: mock.Mock,
    tmp_path: Path,
) -> None:
    """Manifest errors are printed concisely with paths relative to output_dir."""
    output_dir = tmp_path / "out"
    conflict_path = output_dir / "ns1" / "deployment-app.yaml"
    conflict_path.parent.mkdir(parents=True)
    conflict_path.write_text("apiVersion: apps/v1\nkind: Deployment\n")
    cause = ValueError(
        "Configuration conflict: config2 generates files that are already "
        "generated by another config:\n"
        f"  {conflict_path} (generated by config1)"
    )

    with mock.patch(
        "manifest_builder.cli.generate", side_effect=ManifestError("config2", cause)
    ):
        result = CliRunner().invoke(main, ["--output-dir", str(output_dir)])

    assert result.exit_code == 1
    assert (
        "Error processing config2: Configuration conflict: config2 generates files "
        "that are already generated by another config:\n"
        "  ns1/deployment-app.yaml (generated by config1)"
    ) in result.output
