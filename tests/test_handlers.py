...
"""Tests for the config handler interface."""
...
class GreetingConfigHandler(ConfigHandler[GreetingConfig]):
    """A handler for a config type the library does not know about."""
...
def test_generate_manifests_accepts_an_out_of_tree_config_type(tmp_path: Path) -> None:
    """A handler may bring its own config dataclass, unknown to config.py."""
    handler = GreetingConfigHandler(
        [GreetingConfig(name="greeter", namespace="demo", message="hi there")]
    )

    written = generate_manifests([handler], tmp_path / "out", repo_root=tmp_path)
    ...
    assert handler.validated == ["greeter"]


def test_generate_manifests_reports_out_of_tree_validation_failure(
    tmp_path: Path,
) -> None:
    """Validation raised by a custom handler stops generation."""

    class RejectingHandler(GreetingConfigHandler):
        def validate(self, config: GreetingConfig, repo_root: Path) -> None:
            raise ValueError(f"{config.name} is not welcome")

    handler = RejectingHandler([GreetingConfig(name="greeter", namespace="demo")])

    with pytest.raises(ValueError, match="greeter is not welcome"):
        generate_manifests([handler], tmp_path / "out", repo_root=tmp_path)


class _ConcurrencyProbe(GreetingConfigHandler):
...
def test_parallel_safe_handler_generates_concurrently(tmp_path: Path) -> None:
    """A handler opting into parallel_safe has its configs rendered together."""
    configs = [
        GreetingConfig(name=name, namespace="demo") for name in ("first", "second")
    ]
    handler = _ConcurrencyProbe(configs, expected_overlap=2)
    handler.parallel_safe = True
...
def test_handlers_are_sequential_by_default(tmp_path: Path) -> None:
    ...
    handler = _ConcurrencyProbe(configs, expected_overlap=1)

    assert handler.parallel_safe is False
    generate_manifests([handler], tmp_path / "out", repo_root=tmp_path)
